-- Deploy the UPDATE trigger for automatic car wash synchronization
-- This will ensure future changes to rental dropoff dates automatically update the linked car wash
...
BEGIN
  ...
  IF (NEW.dropoff_date != OLD.dropoff_date) THEN
    ...
    IF (NEW.service_type IS NULL OR NEW.service_type = 'rental' OR NEW.service_type = 'car_rental') THEN
      ...
      -- Find the linked car wash booking and store its ID
      SELECT id INTO v_linked_carwash_id
      FROM bookings
      ...
      IF v_linked_carwash_id IS NOT NULL THEN
        ...
        FOR i IN 0..10 LOOP
          ...
          WHERE (service_type = 'car_wash' OR service_type = 'mechanical_service')
            AND status != 'cancelled'
            AND id != v_linked_carwash_id  -- Exclude the booking we're updating
            AND (
              -- Check for overlap
