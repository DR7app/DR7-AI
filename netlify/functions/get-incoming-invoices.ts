...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    // Una sola pagina di doppioni bastava a mostrare la stessa fattura due
    // volte in elenco: la paginazione corretta e la deduplica per filename
    // stanno in fetchAllIncomingInvoices (vedi il commento li').
    const allInvoices = await fetchAllIncomingInvoices({ startDate, endDate, pageSize: 100, maxPages: 50 })
