...
export const handler: Handler = async (event) => {
    ...
    try {
        ...
        if (templateError && templateError.code !== 'PGRST116') {
            console.error('[send-lottery-postponement] Error fetching template:', templateError)
        }

        // Default template (fallback if no custom template exists)
        const defaultSubject = '🎟️ Importante: Estrazione Lotteria DR7 Rinviata al 24 Gennaio 2026 - ULTIMI BIGLIETTI'
        const defaultHtml = `
            <!DOCTYPE html>
            <html>
            <head>
...
Questa è una comunicazione ufficiale riguardante la tua partecipazione alla Lotteria DR7.
        ...
        const emailSubject = template?.subject || defaultSubject
        const emailHtml = template?.html_content || defaultHtml
        const emailText = template?.text_content || defaultText
