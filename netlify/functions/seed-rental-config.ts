...
const CONFIG_DATA = {
  ...
  insurance: {
    exotic: {
      TIER_1: [
        ...
        { id: "KASKO_BASE", name: "Kasko Base", daily_price: 119, deductible: "5000 + 30% del danno" }
      ],
      TIER_2: [
        { id: "RCA", name: "RCA Compresa (no Kasko)", daily_price: 0, mandatory_deposit: 10000 },
        { id: "KASKO_BASE", name: "Kasko Base", daily_price: 89, deductible: "5000 + 30% del danno" },
        { id: "KASKO_BLACK", name: "Kasko Black", daily_price: 149, deductible: "5000 + 10% del danno" },
        { id: "KASKO_SIGNATURE", name: "Kasko Signature", daily_price: 189, deductible: "5000 fisso" },
  ...
  experience_services: [
    ...
    { id: "wedding", name: "Allestimento matrimonio", price: 150, unit: "flat", is_active: true, tier_only: null },
    ...
    { id: "video_drone", name: "Video Maker + Drone", price: 200, unit: "per_hour", is_active: true, tier_only: null },
    { id: "premium_24h", name: "Assistenza premium 24h", price: 19.90, unit: "per_day", is_active: true, tier_only: null },
    { id: "vehicle_replacement", name: "Sostituzione veicolo", price: 19.90, unit: "per_day", is_active: true, tier_only: "TIER_2" },
    { id: "chauffeur_vip", name: "Noleggio con autista VIP", price: 189, unit: "per_hour", is_active: true, tier_only: null }
  ],
  dr7_flex: {
    daily_price: 19.90,
    refund_percent: 90,
    tier_restriction: "TIER_2",
    description: "Cancella fino al giorno del noleggio - rimborso 90% come credito DR7 Wallet"
  },
  rental_day_rates: {
    exotic: {
  ...
  try {
    ...
    if (existing) {
      ...
      const { error } = await supabase
        .from('rental_extras_config')
        .update({ config: CONFIG_DATA, updated_at: new Date().toISOString(), updated_by: 'seed-script' })
        .eq('id', existing.id)
      ...
      const { error } = await supabase
        .from('rental_extras_config')
        .insert({ config: CONFIG_DATA, updated_at: new Date().toISOString(), updated_by: 'seed-script' })
      if (error) throw error
    }
    ...
    return {
      statusCode: 200,
      headers,
      body: JSON.stringify({ success: true, message: 'Config seeded successfully' })
    }
  } catch (err: unknown) {
