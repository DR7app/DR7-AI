-- Fix the INSERT trigger to properly check for existing car wash by source_booking_id
-- This prevents duplicate car washes from being created for the same rental
...
BEGIN
  ...
  IF (NEW.service_type IS NULL OR NEW.service_type = 'rental' OR NEW.service_type = 'car_rental') THEN
    ...
    -- FIXED: Check if we already created a car wash for THIS SPECIFIC RENTAL by source_booking_id
    IF EXISTS (
      ...
      RAISE NOTICE 'Car wash already exists for rental ID %', NEW.id;
      RETURN NEW;
    END IF;
    ...
    RAISE NOTICE 'Auto-created car wash booking for rental % (vehicle %) at %', NEW.id, NEW.vehicle_name, v_next_available_slot;
  END IF;
...
SELECT 'Trigger function updated - now checks by source_booking_id to prevent duplicates' as status;
