-- Deploy the car wash auto-update trigger to production
-- This will make car wash bookings automatically update when you extend a rental
      ...
      IF FOUND THEN
        ...
        FOR i IN 0..10 LOOP
          ...
          WHERE (service_type = 'car_wash' OR service_type = 'mechanical_service')
            AND status != 'cancelled'
            AND id != v_carwash_id  -- Exclude the booking we're updating
            AND (
              -- Check for overlap
