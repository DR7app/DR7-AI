...
as $$
  ...
  select json_build_object(
    ...
    -- Persone distinte con almeno una prenotazione valida, su tutti i servizi.
    'clienti_serviti', (
      select count(distinct coalesce(
        booking_details->'customer'->>'customerId', booking_details->'customer'->>'id', user_id::text,
        'e:' || lower(nullif(trim(customer_email),'')),
        'p:' || right(nullif(regexp_replace(coalesce(customer_phone,''),'\D','','g'),''), 9),
        'n:' || lower(nullif(trim(customer_name),''))))
      from valide),
