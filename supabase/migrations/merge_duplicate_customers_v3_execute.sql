-- ============================================
-- MERGE DUPLICATE CUSTOMERS v3 — EXECUTE
-- Run the PREVIEW script first to verify!
-- This script merges data, updates FKs, deletes duplicates
-- ============================================
...
-- ============================================
-- STEP 1: Identify duplicates by CODICE FISCALE
-- ============================================
...
SELECT
  ...
  (
    ...
    WHERE UPPER(TRIM(ce2.codice_fiscale)) = UPPER(TRIM(ce.codice_fiscale))
      AND ce2.codice_fiscale IS NOT NULL AND TRIM(ce2.codice_fiscale) != ''
...
WHERE ce.codice_fiscale IS NOT NULL AND TRIM(ce.codice_fiscale) != ''
  AND LENGTH(TRIM(ce.codice_fiscale)) >= 11
  AND EXISTS (
    SELECT 1 FROM customers_extended ce2
    WHERE UPPER(TRIM(ce2.codice_fiscale)) = UPPER(TRIM(ce.codice_fiscale)) AND ce2.id != ce.id
  );

-- ============================================
-- STEP 2: Identify duplicates by PARTITA IVA
-- ============================================
INSERT INTO customer_merge_map (duplicate_id, primary_id, merge_reason)
SELECT ce.id,
  (
...
WHERE ce.partita_iva IS NOT NULL AND TRIM(ce.partita_iva) != ''
  ...
  AND NOT EXISTS (SELECT 1 FROM customer_merge_map WHERE duplicate_id = ce.id);

-- ============================================
-- STEP 3: Identify duplicates by EMAIL
-- ============================================
INSERT INTO customer_merge_map (duplicate_id, primary_id, merge_reason)
SELECT ce.id,
  (
...
-- ============================================
-- STEP 4: Identify duplicates by PHONE
-- ============================================
INSERT INTO customer_merge_map (duplicate_id, primary_id, merge_reason)
SELECT ce.id,
  (
...
-- ============================================
-- STEP 5: Identify duplicates by EXACT NAME (nome + cognome)
-- ============================================
INSERT INTO customer_merge_map (duplicate_id, primary_id, merge_reason)
SELECT ce.id,
  (
...
-- Show count
SELECT merge_reason, COUNT(*) as count FROM customer_merge_map GROUP BY merge_reason ORDER BY count DESC;
SELECT COUNT(*) as total_duplicates FROM customer_merge_map;
...
-- ============================================
-- UPDATE FOREIGN KEYS (all tables referencing customer_id)
-- ============================================
...
-- 2. Commercial operation tickets
...
-- 4. Birthday discount codes (no UNIQUE constraint on customer_id — safe to update)
UPDATE birthday_discount_codes
SET customer_id = cmm.primary_id
FROM customer_merge_map cmm
WHERE birthday_discount_codes.customer_id = cmm.duplicate_id;

-- 5. Birthday vouchers (no UNIQUE constraint — safe to update)
UPDATE birthday_vouchers
SET customer_id = cmm.primary_id
FROM customer_merge_map cmm
WHERE birthday_vouchers.customer_id = cmm.duplicate_id;

-- 6. Customer memberships (uses client_id, not customer_id)
UPDATE customer_memberships
SET client_id = cmm.primary_id
FROM customer_merge_map cmm
WHERE customer_memberships.client_id = cmm.duplicate_id;

-- ============================================
-- HANDLE TABLES WITH UNIQUE(customer_id) CONSTRAINTS
-- These need special handling: delete duplicate's row if primary already has one
-- ============================================

-- 7. Birthday messages — UNIQUE(customer_id, year)
-- Delete duplicate's messages where primary already has one for that year
DELETE FROM birthday_messages bm
USING customer_merge_map cmm
WHERE bm.customer_id = cmm.duplicate_id
  AND EXISTS (
    SELECT 1 FROM birthday_messages bm2
    WHERE bm2.customer_id = cmm.primary_id AND bm2.year = bm.year
  );
-- Now safe to update remaining
UPDATE birthday_messages
SET customer_id = cmm.primary_id
FROM customer_merge_map cmm
WHERE birthday_messages.customer_id = cmm.duplicate_id;

-- 8. Review WhatsApp sent — UNIQUE(customer_id)
-- Delete duplicate's row if primary already has one
DELETE FROM review_whatsapp_sent rws
USING customer_merge_map cmm
WHERE rws.customer_id = cmm.duplicate_id
  AND EXISTS (
    SELECT 1 FROM review_whatsapp_sent rws2
    WHERE rws2.customer_id = cmm.primary_id
  );
-- Now safe to update remaining
...
-- 9. Customer wallets — UNIQUE(customer_id)
-- If both have wallets, merge balances into primary and delete duplicate wallet
UPDATE customer_wallets cw_primary
SET
  balance_cents = cw_primary.balance_cents + cw_dup.balance_cents,
  total_earned_cents = cw_primary.total_earned_cents + cw_dup.total_earned_cents,
  total_spent_cents = cw_primary.total_spent_cents + cw_dup.total_spent_cents,
  updated_at = NOW()
FROM customer_wallets cw_dup
INNER JOIN customer_merge_map cmm ON cw_dup.customer_id = cmm.duplicate_id
WHERE cw_primary.customer_id = cmm.primary_id;

-- Move transactions from duplicate wallet to primary wallet
UPDATE customer_wallet_transactions cwt
SET wallet_id = cw_primary.id
FROM customer_wallets cw_dup
INNER JOIN customer_merge_map cmm ON cw_dup.customer_id = cmm.duplicate_id
INNER JOIN customer_wallets cw_primary ON cw_primary.customer_id = cmm.primary_id
WHERE cwt.wallet_id = cw_dup.id;

-- Delete duplicate wallets where primary already has one
DELETE FROM customer_wallets cw
USING customer_merge_map cmm
WHERE cw.customer_id = cmm.duplicate_id
  AND EXISTS (
    SELECT 1 FROM customer_wallets cw2
    WHERE cw2.customer_id = cmm.primary_id
  );

-- Now safe to update remaining wallets (where primary didn't have one)
UPDATE customer_wallets
SET customer_id = cmm.primary_id
FROM customer_merge_map cmm
WHERE customer_wallets.customer_id = cmm.duplicate_id;
...
-- Delete from customers table (backward compat)
