-- ============================================
-- MERGE DUPLICATE CUSTOMERS - ENHANCED
-- Merges by: email, phone, codice_fiscale, AND name+email combination
-- ============================================
...
-- ============================================
-- STEP 1: Merge by SAME NAME + SAME EMAIL (case insensitive)
-- ============================================
...
SELECT 
  ...
  (
    ...
    WHERE LOWER(ce2.email) = LOWER(ce.email)
      AND LOWER(TRIM(ce2.nome)) = LOWER(TRIM(ce.nome))
      AND LOWER(TRIM(ce2.cognome)) = LOWER(TRIM(ce.cognome))
      AND ce2.email IS NOT NULL AND ce2.email != ''
      AND ce2.nome IS NOT NULL AND ce2.nome != ''
      AND ce2.cognome IS NOT NULL AND ce2.cognome != ''
  ...
  'duplicate_name_email' as merge_reason
FROM customers_extended ce
WHERE ce.email IS NOT NULL AND ce.email != ''
  AND ce.nome IS NOT NULL AND ce.nome != ''
  AND ce.cognome IS NOT NULL AND ce.cognome != ''
...
-- ============================================
-- STEP 2: Merge by EMAIL only (different names OK)
-- ============================================
INSERT INTO customer_merge_map (duplicate_id, primary_id, merge_reason)
SELECT 
  ce.id as duplicate_id,
  (
    ...
    ORDER BY 
      (CASE WHEN ce2.nome IS NOT NULL AND ce2.nome != '' THEN 1 ELSE 0 END +
       ...
       CASE WHEN ce2.codice_fiscale IS NOT NULL AND ce2.codice_fiscale != '' THEN 1 ELSE 0 END +
       CASE WHEN ce2.telefono IS NOT NULL AND ce2.telefono != '' THEN 1 ELSE 0 END +
       CASE WHEN ce2.indirizzo IS NOT NULL AND ce2.indirizzo != '' THEN 1 ELSE 0 END) DESC,
      ce2.created_at ASC
    LIMIT 1
  ) as primary_id,
...
SELECT 
  ...
  (
    ...
    WHERE ce2.telefono = ce.telefono
      AND ce2.telefono IS NOT NULL AND ce2.telefono != ''
    ...
    ORDER BY 
      (CASE WHEN ce2.nome IS NOT NULL AND ce2.nome != '' THEN 1 ELSE 0 END +
       ...
       CASE WHEN ce2.codice_fiscale IS NOT NULL AND ce2.codice_fiscale != '' THEN 1 ELSE 0 END) DESC,
      ce2.created_at ASC
    LIMIT 1
  ) as primary_id,
  'duplicate_phone' as merge_reason
FROM customers_extended ce
WHERE ce.telefono IS NOT NULL AND ce.telefono != ''
  ...
  AND EXISTS (
    SELECT 1 
    FROM customers_extended ce2 
    WHERE ce2.telefono = ce.telefono AND ce2.id != ce.id
  )
  AND NOT EXISTS (
    SELECT 1 FROM customer_merge_map WHERE duplicate_id = ce.id
  );
...
-- ============================================
-- STEP 4: Merge by CODICE FISCALE
-- ============================================
INSERT INTO customer_merge_map (duplicate_id, primary_id, merge_reason)
SELECT 
  ce.id as duplicate_id,
  (
    ...
    WHERE UPPER(ce2.codice_fiscale) = UPPER(ce.codice_fiscale)
      AND ce2.codice_fiscale IS NOT NULL AND ce2.codice_fiscale != ''
...
FROM customers_extended ce
WHERE ce.codice_fiscale IS NOT NULL AND ce.codice_fiscale != ''
  AND LENGTH(ce.codice_fiscale) = 16
  AND EXISTS (
    SELECT 1 
    FROM customers_extended ce2 
    WHERE UPPER(ce2.codice_fiscale) = UPPER(ce.codice_fiscale) AND ce2.id != ce.id
  )
  AND NOT EXISTS (
    SELECT 1 FROM customer_merge_map WHERE duplicate_id = ce.id
  );
