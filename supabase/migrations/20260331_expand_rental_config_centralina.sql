-- Centralina Unica: Expand rental_extras_config with comprehensive pricing config
-- This migration seeds the config JSONB with all current hardcoded values
-- so that admin can manage everything from one place.

-- Update the existing singleton row with the full config structure
UPDATE rental_extras_config SET config = '{
  "schema_version": 2,
  ...
  "tier_rules": {
    ...
    "TIER_1": {
      "label": "Fascia B — Conducente giovane o patente recente",
      "age_range": [21, 25],
      "license_years_range": [3, 4]
    },
    "TIER_2": {
      "label": "Fascia A — Conducente esperto",
      "min_age": 26,
      "max_age": 69,
      "min_license_years": 5
    }
