-- Seed ALL active system message templates with EXACT text currently sent
...
-- ═══ Recensioni (review-send.ts — the one actually triggered) ═══

('review_request_whatsapp', 'Richiesta Recensione', 'Inviato dopo la riconsegna per chiedere una recensione',
 'Ciao {customer_name}! 👋
...
-- ═══ Promemoria Firma (signature-reminder cron) ═══
...
-- ═══ Contratto Firmato Admin Alert (signature-complete) ═══
...
-- ═══ Pagamento Danni/Penali (nexi-payment-callback) ═══

('payment_received_damages', 'Conferma Pagamento Danni/Penali (cliente)', 'Inviato al cliente dopo pagamento danni o penali',
 'Gentile {custName},
...
('payment_received_damages_admin', 'Alert Admin: Pagamento Danni/Penali', 'Notifica admin dopo pagamento danni o penali ricevuto',
 '💰 *PAGAMENTO {paymentType} RICEVUTO*
...
-- ═══ Pagamento Estensione (nexi-payment-callback) ═══

('payment_received_extension', 'Conferma Pagamento Estensione (cliente)', 'Inviato al cliente dopo pagamento estensione noleggio',
 'Gentile {custName},
...
('payment_received_extension_admin', 'Alert Admin: Pagamento Estensione', 'Notifica admin dopo pagamento estensione ricevuto',
 '💰 *PAGAMENTO ESTENSIONE RICEVUTO*
...
-- ═══ Wallet Bonus (nexi-payment-callback) ═══

('wallet_bonus_credit', 'Credito Wallet Bonus (cliente)', 'Inviato al cliente quando riceve credito wallet da pagamento carta',
 'Gentile {custName},
...
('wallet_bonus_credit_admin', 'Alert Admin: Bonus Carta', 'Notifica admin quando viene accreditato un bonus carta',
 '*BONUS CARTA ACCREDITATO*

*Cliente:* {customer_name}
*Carta:* {cardLabel}
*Bonus:* €{bonusEur} ({percentLabel})
*Nuovo saldo wallet:* €{newBalance}
...
('prepaid_card_blocked_admin', 'Alert Admin: Carta Prepagata', 'Notifica admin carta prepagata bloccata',
...
-- ═══ Fattura PDF (generate-invoice/penalty) ═══

('invoice_pdf_whatsapp', 'Invio Fattura PDF', 'Caption del PDF fattura inviato via WhatsApp',
 'Fattura {numero_fattura} - DR7 Empire',
 true, true, 'on_payment', 'all', 'confirmed'),

('penalty_invoice_pdf_whatsapp', 'Invio Fattura Penale PDF', 'Caption del PDF fattura penale',
 'Fattura {numero_fattura} - DR7 Empire',
 true, true, 'on_payment', 'all', 'confirmed'),
...
-- ═══ Membership Reminder (send-membership-reminders.js) ═══

('membership_renewal_reminder', 'Promemoria Rinnovo Membership', 'Inviato 3 giorni prima della scadenza membership',
 'Ciao {displayName}!

La tua membership *{tier_name}* scade il *{formattedExpiry}* (tra {daysLeft} giorni).
