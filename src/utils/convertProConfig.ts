...
export function convertProToRentalConfig(pro: ProSnapshot): RentalConfig {
  ...
  if (pro.km) {
    ...
    for (const kmCfg of pro.km) {
      ...
      // 2026-05-22: se il toggle e' ON salviamo SEMPRE l'entry, anche con
      // prezzo 0 (admin puo' attivare Km illimitati come "Incluso/Gratis").
      // Distinzione lato website: entry assente = toggle OFF, entry presente
      // con 0 = toggle ON gratis, entry presente con valore = toggle ON a pagamento.
      ...
      if (unlimitedEnabled && unlMode === 'per_fascia' && kmCfg.unlimitedByFascia) {
        ...
        unlimitedKm[dbCat] = {
          TIER_2: { per_day: faA },
          TIER_1: { per_day: faB },
        } as (typeof unlimitedKm)[string]
      } else if (unlimitedEnabled) {
      ...
      if (Array.isArray(pkgsRaw) && pkgsRaw.length > 0) {
        const pkgs = pkgsRaw
          ...
          .map(p => {
            ...
            // 2026-05-17: pacchetti >=300 km SEMPRE quantity-buyable, con
            // max_quantity 99 (effettivamente illimitato — direzione vuole
            // poter aggiungere 300 km piu' volte senza tetto).
            const isQty = km >= 300 || p.is_quantity_buyable === true
            const maxQ = isQty ? Math.max(99, num(p.max_quantity) || 99) : 1
