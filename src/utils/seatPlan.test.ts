...
describe('seatPlan', () => {
  ...
  it('dietro e- un divano: i blocchi coprono tutti i sedili, senza doppioni', () => {
    ...
    expect(SEAT_BLOCKS.filter(b => b.row === 3).every(b => b.seats.length === 1)).toBe(true)
    expect(SEAT_BLOCKS.filter(b => b.seats.length > 1).map(b => b.seats)).toEqual([['PS', 'PC', 'PD']])
  })
  ...
  it('il divano intero e- UNA voce nei riepiloghi, non tre', () => {
    ...
    expect(seatListLabel(['TS', 'TD'])).toBe('Terza fila sinistra, Terza fila destra')
