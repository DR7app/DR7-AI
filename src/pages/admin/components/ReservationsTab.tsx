...
export default function ReservationsTab({ initialData, onDataConsumed, viewMode = 'bookings', serviceType = 'rental' }: { initialData?: { vehicleId?: string; pickupDate?: Date; bookingId?: string; fromPreventivo?: Record<string, any> } | null; onDataConsumed?: () => void; viewMode?: 'bookings' | 'uscite'; serviceType?: string }) {
  ...
  // 15/09/2026: scegliendo "Credit Wallet" come metodo di pagamento, l'importo
  // esce davvero dal wallet del cliente — lo fa il database al salvataggio
  // (trigger trg_dr7_wallet_sync_prenotazione). Qui si mostra il saldo PRIMA,
  // perche' l'operatore deve sapere cosa sta per prelevare e da chi. Si legge
  ...
  */
  ...
  async function processBookingSubmission(skipValidation = false, overrideCustomerId?: string) {
    ...
    try {
      ...
      // ===== VALIDATION: Credit Wallet — credito sufficiente =====
      // 15/09/2026 (direzione): salvando con metodo "Credit Wallet" l'importo
      // esce davvero dal wallet del cliente (trigger di database). Il Credit
      // Wallet NON puo' andare in negativo: se il saldo non basta la
      // prenotazione non si salva, punto. Qui non c'e' OTP di proposito — con
      // il toggle spento in Gestione OTP si auto-approverebbe in silenzio, ed
      // e' cosi' che era passata la prima prenotazione senza copertura.
      if (isCreditWallet(formData.payment_method)) {
      ...
      // L'addebito lo ha gia' fatto il database insieme al salvataggio
      // (trigger trg_dr7_wallet_sync_prenotazione): qui si rilegge il registro
      // dei movimenti per dirlo all'operatore con l'importo reale, non con
      // quello che la schermata si aspettava. Non blocca nulla: la
      // prenotazione e' salvata comunque.
