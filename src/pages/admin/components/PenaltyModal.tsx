            ...
            if (paymentStatus === 'paid') {
                ...
                try {
                    ...
                    if (linkRes.ok && linkData.paymentUrl) {
                        // 2026-05-30: invia SOLO il link al cliente della prenotazione
                        // via WhatsApp, usando il template custom scelto dalla direzione
                        // (Messaggi di Sistema Pro). Nessun altro messaggio.
                        try { await navigator.clipboard.writeText(linkData.paymentUrl) } catch { /* clipboard not available */ }
                        const custPhone = (currentBookingNexi as { customer_phone?: string } | null)?.customer_phone || (booking as { customer_phone?: string }).customer_phone || currentBookingNexi?.booking_details?.customer?.phone
                        ...
                        if (sent) toast.success(`Link pagamento inviato al cliente via WhatsApp (€${cartTotal.toFixed(2)})`)
                        else toast(`Link creato (€${cartTotal.toFixed(2)}) — copiato negli appunti${custPhone ? ' (invio WhatsApp non riuscito, invialo a mano)' : ' (cliente senza numero)'}`, { icon: '⚠️', duration: 8000 })
                    } else {
