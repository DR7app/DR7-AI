...
export default function MessaggiSistemaProTab() {
    ...
    async function handleSaveEdit(id: string) {
        ...
        try {
            ...
            const edited = templates.find(t => t.id === id)
            ...
            logAdminAction('system_message_updated', 'system_message', id, {
                template: edited?.message_key,
                label: edited?.label,
                body_preview: editBody.slice(0, 120),
            })
        } catch (err: unknown) {
            const _errMsg = err instanceof Error ? err.message : String(err)
    ...
    async function handleCreateTemplate() {
        ...
        try {
            ...
            logAdminAction('system_message_created', 'system_message', data.id, {
                template: messageKey,
                label: newLabel.trim(),
            })
    ...
    async function handleToggleAutomatic(template: SystemMessage) {
        try {
            ...
            logAdminAction('system_message_toggled', 'system_message', template.id, {
                template: template.message_key,
    ...
    async function handleSend() {
            ...
            try {
                ...
                if (response.ok && result.success) {
                    ...
                    const templateLabel = sendMode === 'template'
                        ...
                        : null
                    const templateKey = sendMode === 'template'
                        ? templates.find(t => t.id === selectedTemplateId)?.message_key || null
                        : null
                    ...
                    if (sendMode === 'template') {
                        logAdminAction('whatsapp_sent', 'customer', customer.id, {
                            template: templateKey || templateLabel,
        ...
        if (sendMode === 'free' && successCount > 0) {
            logAdminAction('whatsapp_free_text', undefined, undefined, {
                recipients_count: successCount,
                failed_count: failCount || undefined,
                preview: messageText.slice(0, 120),
            })
        } else if (sendMode === 'template' && customersWithPhone.length > 1) {
            const templateKey = templates.find(t => t.id === selectedTemplateId)?.message_key || null
            logAdminAction('whatsapp_bulk_send', undefined, undefined, {
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="space-y-4">
                ...
                {/* Template Cards — Grouped by KIND (not by service) */}
                <div className="space-y-6">
                    {PRO_MESSAGE_CATEGORIES.map(cat => {
                    ...
                    {/* Custom Pro messages (pro_custom_* from "+ Nuovo Messaggio") */}
                    {(() => {
                        const categorizedKeys = new Set(ALL_PRO_TEMPLATES.map(t => t.key))
                        const custom = templates.filter(t => !categorizedKeys.has(t.message_key))
