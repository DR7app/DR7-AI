...
const TRIGGER_LABELS: Record<string, string> = {
    ...
    'on_acconto': 'Acconto registrato (collaboratore)',
...
const EVENT_GROUPS: Array<{ label: string; color: string; keys: string[]; area: EventArea; service?: 'rental' | 'car_wash' | 'mechanical' }> = [
  ...
  // ── PAGAMENTI (Amministrazione > Attesa di pagamento) ───────────────────
  {
    ...
    keys: [
      ...
      // 2026-08-14: aveva la descrizione ma non stava in nessun gruppo,
      // quindi non compariva nell'elenco e non era selezionabile.
      'sollecito_pagamento',
...
const TRIGGER_DESCRIPTIONS: Record<string, string> = {
    ...
    'on_acconto': 'Parte SUBITO quando l\'amministrazione registra un acconto nella tab Acconti. Va al COLLABORATORE intestatario dell\'acconto, sul numero scritto in \"Contatto interno\" della sua scheda Operatori (nessun cliente lo riceve). Variabili: {acconto}/{importo}, {causale}, {metodo}, {data_acconto}, {operatore}.',
    ...
    'before_uscita': 'Il messaggio parte PRIMA della partenza di un\'uscita straordinaria e va agli AUTISTI assegnati a quella uscita (numero della loro scheda). Offset in ore: 2 = due ore prima. Filtro "Tipo servizio" = business dell\'uscita (Terra, Mare, Aria, Soggiorni, Lavaggio). Variabili: {nome_autista} (o {nome}), {titolo_corsa}, {veicolo} (o {vehicle_name}), {targa} (o {vehicle_plate}), {data_ritiro} (o {pickup_date}), {ora_ritiro} (o {pickup_time}), {luogo_ritiro} (o {pickup_location}), {indirizzo_ritiro}, {data_riconsegna}, {ora_riconsegna}, {luogo_riconsegna}, {motivazione_uscita}, {note_operative}. Una variabile scritta con un nome che non esiste blocca l\'invio: il messaggio non parte e non arriva a nessuno.',
...
const TEMPLATE_VAR_GROUPS: VarGroup[] = [
    ...
    },
    {
        ...
        items: [
            { key: 'rental_line', description: 'Riga noleggio completa', example: '6 giorni = €894,00' },
    ...
    },
    {
        ...
        items: [
            ...
            { key: 'nome_autista', description: "Nome dell'autista (anche {nome} o {customer_name})", example: 'Luca' },
            { key: 'veicolo', description: 'Veicolo assegnato (modello) — anche {vehicle_name}', example: 'Audi RS3' },
            { key: 'targa', description: 'Targa del veicolo — anche {vehicle_plate}', example: 'AB123CD' },
            { key: 'data_ritiro', description: 'Data ritiro — anche {pickup_date}', example: '12/05/2026' },
            { key: 'ora_ritiro', description: 'Ora ritiro — anche {pickup_time}', example: '11:00' },
            { key: 'luogo_ritiro', description: 'Luogo ritiro (nome) — anche {pickup_location}', example: 'DR7 Office Cagliari' },
            { key: 'indirizzo_ritiro', description: 'Indirizzo ritiro — anche {pickup_address}', example: 'Viale Marconi 229' },
            { key: 'data_riconsegna', description: 'Data riconsegna — anche {dropoff_date}', example: '15/05/2026' },
            { key: 'ora_riconsegna', description: 'Ora riconsegna — anche {dropoff_time}', example: '10:00' },
            { key: 'luogo_riconsegna', description: 'Luogo riconsegna (nome) — anche {dropoff_location}', example: 'Aeroporto Cagliari Elmas' },
            { key: 'indirizzo_riconsegna', description: 'Indirizzo riconsegna — anche {dropoff_address}', example: '—' },
