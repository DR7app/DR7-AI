            ...
            <div className="flex gap-1.5 justify-end items-center flex-wrap">
              {isEligible && (
                <>
                  ...
                  {/* 17/09/2026 (direzione): sulla riga bloccata i bottoni restano
                      visibili ma grigi e disattivati. */}
                  <button
                    onClick={() => { if (!bloccato) handleCopiaMessaggio(candidate) }}
                    disabled={bloccato || copyingId === candidate.id}
                    title={bloccato ? 'Cliente bloccato: sbloccalo per copiare il messaggio' : 'Copia il messaggio negli appunti (non invia nulla)'}
                    className="inline-flex items-center justify-center w-8 h-8 rounded-full border border-theme-border text-theme-text-secondary hover:bg-theme-bg-hover transition-colors disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-transparent"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  ...
                  {(candidate.customer_email || candidate.customer_phone) && (
                    <button
                      onClick={() => { if (!bloccato) handleGenerateAndSendCode(candidate) }}
                      disabled={bloccato || generatingCodeId === candidate.id}
                      title={bloccato ? 'Cliente bloccato: sbloccalo per inviare un codice' : 'Genera codice sconto reale (Supercar EUR 100 + Lavaggio EUR 10) e invialo via WhatsApp'}
                      className={`inline-flex items-center justify-center w-8 h-8 rounded-full border border-theme-border transition-colors ${
                        bloccato
                          ? 'bg-theme-bg-tertiary text-theme-text-muted cursor-not-allowed'
                          : 'bg-dr7-gold text-white hover:opacity-90 disabled:opacity-50'
                      }`}
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <td className="px-4 py-3 text-theme-text-secondary">
                {candidate.customer_phone ? (
                  <span className="inline-flex items-center gap-2">
                    ...
                    <span><NumeroTelefono valore={candidate.customer_phone} /></span>
                  </span>
                ) : (
                  <span className="text-theme-text-secondary/60">—</span>
                )}
