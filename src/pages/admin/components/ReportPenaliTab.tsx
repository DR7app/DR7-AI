...
export default function ReportPenaliTab() {
  ...
  async function reloadOv() { setOverrides(await loadReportOverrides('penali')) }
  async function saveEdit(changes: Record<string, number>, note: string) {
    if (!editRow) return
    for (const [field, value] of Object.entries(changes)) await saveEditOverride('penali', rowKeyOf(editRow), field, value, note)
  ...
  async function removeRow(v: any) {
    ...
    if (v._isManual) await deleteOverrideById(v._manualId); else await saveRemoveOverride('penali', rowKeyOf(v), null)
    await reloadOv(); toast.success('Voce rimossa')
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  async function restoreRow(v: any) {
    if (v._isManual) await deleteOverrideById(v._manualId); else await deleteOverrideByRow('penali', rowKeyOf(v))
  ...
  async function fetchReport() {
    ...
    setError('')
    try {
      const res = await fetch('/.netlify/functions/report-danni?type=penali')
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
        <div>
          <h2 className="text-2xl font-bold text-theme-text-primary tracking-tight">Report Penali</h2>
          <p className="text-sm text-theme-text-secondary mt-0.5">Riepilogo delle penali contrattuali applicate per veicolo</p>
        </div>
        <button
          onClick={fetchReport}
          disabled={loading}
      ...
      {data && (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
            <KpiCard label="Veicoli con Penali" value={`${data.totalVehicles}`} sub="parco coinvolto" accent="amber" />
            <KpiCard label="Penali Applicate" value={`${data.totalCount}`} sub={`${data.totalCount === 1 ? 'caso' : 'casi'}`} accent="orange" />
            <KpiCard label="Importo Totale" value={formatCurrency(data.totalAmount)} sub={`${formatCurrency(avgPerVehicle)} medio`} accent="gold" />
          </div>

          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-orange-50 border border-orange-200 text-orange-700 text-sm font-medium">
              <span className="w-1.5 h-1.5 rounded-full bg-orange-500" />
              Penali
              <span className="text-xs text-orange-600/70">{data.totalCount}</span>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="lg:col-span-2 bg-theme-bg-primary border border-theme-border rounded-2xl overflow-hidden shadow-sm">
              <div className="px-5 py-4 border-b border-theme-border">
                <h3 className="text-base font-semibold text-theme-text-primary">Lista Penali per Veicolo</h3>
              ...
              {!loading && sorted.length === 0 && (
                <div className="px-5 py-12 text-center text-theme-text-muted text-sm">Nessuna penale registrata.</div>
              )}
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {pageItems.map((v, i) => (
                          <tr key={v.vehiclePlate || i} className="border-b border-theme-border last:border-0 hover:bg-theme-bg-hover/40 transition-colors">
                            ...
                            <td className="px-5 py-3 text-center font-semibold text-orange-600">{v.count}</td>
                      ...
                      <tfoot>
                        <tr className="bg-theme-bg-tertiary/30 border-t border-theme-border">
                          ...
                          <td className="px-5 py-3 text-center font-bold text-orange-600">{data.totalCount}</td>
                    ...
                    {pageItems.map((v, i) => (
                      <div key={v.vehiclePlate || i} className="px-4 py-3">
                        <div className="flex items-start justify-between gap-2 mb-2">
                          ...
                          <span className="shrink-0 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border bg-orange-50 text-orange-700 border-orange-200">
                            {v.count}
                          </span>
                        </div>
                        <div className="flex items-center justify-between gap-2">
                      ...
                      <div key={c.name} className="flex items-center justify-between gap-3 py-1.5 border-b border-theme-border last:border-0">
                        <div className="flex items-center gap-2 min-w-0 flex-1">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-theme-text-muted">{c.count} {c.count === 1 ? 'penale' : 'penali'}</p>
                          </div>
                        </div>
  ...
  return (
    <div className="bg-theme-bg-primary border border-theme-border rounded-2xl p-4 shadow-sm">
      <div className="flex items-center justify-between mb-2">
        ...
        <span className={`inline-flex w-2 h-2 rounded-full ${dot[accent]}`} />
      </div>
