...
export default function CustomerWalletTab() {
  ...
  // Un credito che vale SOLO su certi servizi e SOLO fino a una data. Non
  // entra nel saldo: vive nei suoi lotti e si consuma per primo, il piu'
  // vicino a scadere per primo. Si puo' dare a uno, a una selezione o a tutti.
  ...
  const [vincolatoAperto, setVincolatoAperto] = useState(false)
  const [vincolatoDestinatari, setVincolatoDestinatari] = useState<'cliente' | 'selezione' | 'tutti'>('selezione')
  const [vincolatoSelezione, setVincolatoSelezione] = useState<Set<string>>(new Set())
  const [vincolatoRicerca, setVincolatoRicerca] = useState('')
  const [vincolatoImporto, setVincolatoImporto] = useState('')
  const [vincolatoScadenza, setVincolatoScadenza] = useState('')
  const [vincolatoServizi, setVincolatoServizi] = useState<Set<string>>(new Set())
  const [vincolatoDescrizione, setVincolatoDescrizione] = useState('')
  const [vincolatoInvio, setVincolatoInvio] = useState(false)
  ...
  async function assegnaCreditoVincolato() {
    const importo = parseFloat(String(vincolatoImporto).replace(',', '.'))
    if (!Number.isFinite(importo) || importo <= 0) { toast.error('Inserisci un importo'); return }
    if (vincolatoDestinatari === 'selezione' && vincolatoSelezione.size === 0) {
      toast.error('Scegli almeno un cliente'); return
    }

    // Un regalo a tutti e' una scrittura di massa: si dice prima quanti sono e
    // quanto costa, e si chiede conferma. Chi preme deve poter confrontare il
    // numero con quello che si aspettava.
    const quanti = vincolatoDestinatari === 'tutti'
      ? allWalletCustomers.length
      : vincolatoDestinatari === 'selezione' ? vincolatoSelezione.size : 1
    const servizioTesto = vincolatoServizi.size === 0
      ? 'tutti i servizi'
      : SERVIZI_WALLET.filter(s => vincolatoServizi.has(s.id)).map(s => s.label).join(', ')
    ...
    try {
      const res = await authFetch('/.netlify/functions/customer-wallet-admin', {
        ...
        body: JSON.stringify({
          ...
          amount: importo,
          scadenza: vincolatoScadenza || null,
          servizi: [...vincolatoServizi],
          description: vincolatoDescrizione,
          customer_id: vincolatoDestinatari === 'cliente' ? modalCustomer?.id : undefined,
          destinatari: {
            tipo: vincolatoDestinatari,
            customer_ids: [...vincolatoSelezione],
          },
        }),
      })
      const data = await res.json().catch(() => ({}))
      if (!res.ok || data.error) throw new Error(data.error || `HTTP ${res.status}`)
      toast.success(`Credito assegnato a ${data.destinatari} clienti (EUR ${Number(data.totale).toFixed(2)} in totale)`)
      setVincolatoAperto(false)
      setVincolatoImporto('')
      setVincolatoDescrizione('')
      setVincolatoSelezione(new Set())
    } catch (err: unknown) {
      toast.error('Errore: ' + (err instanceof Error ? err.message : String(err)))
    } finally {
      setVincolatoInvio(false)
    }
  }
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      ...
      {/* ── Credito vincolato ────────────────────────────────────────────── */}
      <div className="rounded-xl border border-theme-border bg-theme-bg-secondary">
        <button
          onClick={() => setVincolatoAperto(v => !v)}
        ...
        {vincolatoAperto && (
          <div className="px-5 pb-5 space-y-4 border-t border-theme-border pt-4">
            {/* Destinatari */}
            <div>
              <div className="text-xs text-theme-text-secondary mb-2">A chi</div>
              <div className="flex flex-wrap gap-2">
                {([
                  ['selezione', 'Clienti scelti'],
                  ['tutti', `Tutti i clienti (${allWalletCustomers.length})`],
                ] as const).map(([id, label]) => (
                  <button
                    key={id}
                    onClick={() => setVincolatoDestinatari(id)}
                    className={`px-3 py-1.5 rounded-lg text-xs border transition-colors ${
                      vincolatoDestinatari === id
              ...
              {vincolatoDestinatari === 'tutti' && (
                <p className="text-[11px] text-amber-400 mt-2">
                  Solo chi ha un account sito riceve il credito: senza account non esiste un wallet da riempire.
                </p>
              )}
            </div>

            {/* Elenco clienti da spuntare */}
            {vincolatoDestinatari === 'selezione' && (
              <div>
                <div className="flex items-center justify-between gap-3 mb-2">
                  <input
                    type="text"
                    value={vincolatoRicerca}
                    onChange={e => setVincolatoRicerca(e.target.value)}
                  ...
                  <span className="text-xs text-theme-text-muted whitespace-nowrap">
                    {vincolatoSelezione.size} scelti
                  </span>
                </div>
                <div className="max-h-56 overflow-y-auto border border-theme-border rounded-lg divide-y divide-theme-border/60">
                  {allWalletCustomers
                    .filter(c => {
                      const q = vincolatoRicerca.trim().toLowerCase()
                      if (!q) return true
                      return (c.full_name || '').toLowerCase().includes(q)
                        || (c.email || '').toLowerCase().includes(q)
                    })
                    .slice(0, 200)
                    .map(c => (
                      <label key={c.id} className="flex items-center gap-3 px-3 py-2 cursor-pointer hover:bg-theme-bg-hover">
                        <input
                          type="checkbox"
                          checked={vincolatoSelezione.has(c.id)}
                          onChange={e => setVincolatoSelezione(prev => {
                        ...
                        />
                        <span className="text-sm text-theme-text-primary flex-1">{c.full_name || 'Senza nome'}</span>
                        <span className="text-[11px] text-theme-text-muted">{c.email}</span>
                      </label>
                    ))}
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  type="text"
                  inputMode="decimal"
                  value={vincolatoImporto}
                  onChange={e => setVincolatoImporto(e.target.value)}
                  placeholder="50,00"
              ...
              <div>
                ...
                <input
                  type="date"
                  value={vincolatoScadenza}
                  onChange={e => setVincolatoScadenza(e.target.value)}
                  className="w-full bg-theme-bg-tertiary border border-theme-border rounded-lg px-3 py-2 text-sm text-theme-text-primary outline-none focus:border-emerald-500/50"
                />
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {SERVIZI_WALLET.map(s => (
                  <button
                    key={s.id}
                    onClick={() => setVincolatoServizi(prev => {
                      const next = new Set(prev)
                      if (next.has(s.id)) next.delete(s.id); else next.add(s.id)
                      return next
                    })}
                    className={`px-3 py-1.5 rounded-lg text-xs border transition-colors ${
                      vincolatoServizi.has(s.id)
                        ? 'bg-emerald-500/15 border-emerald-500/40 text-emerald-300'
                        : 'bg-theme-bg-tertiary border-theme-border text-theme-text-secondary hover:bg-theme-bg-hover'
                    }`}
                  >
              ...
              <p className="text-[11px] text-theme-text-muted mt-2">
                {vincolatoServizi.size === 0
                  ? 'Nessun servizio scelto = vale su tutto (resta il vincolo della scadenza).'
            ...
            <div>
              ...
              <input
                type="text"
                value={vincolatoDescrizione}
                onChange={e => setVincolatoDescrizione(e.target.value)}
            ...
            <button
              onClick={assegnaCreditoVincolato}
