...
export default function CargosTab() {
    ...
    // Sub-tab
    const [activeSubTab, setActiveSubTab] = useState<'send' | 'multe' | 'export'>('send')

    // Multa state
    const [multaFile, setMultaFile] = useState<File | null>(null)
    const [multaPdfBase64, setMultaPdfBase64] = useState<string>('')
    ...
    async function handleProcessMulta() {
        if (!multaPdfBase64) {
            toast.error('Carica prima un PDF')
            return
        }
        ...
        try {
            ...
            if (data.error) {
                ...
                // Still show extracted data if available
    ...
    async function handleSendPec() {
        ...
        try {
            ...
            if (data.error) {
                toast.error('Errore invio PEC: ' + data.error)
                return
            }
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="flex bg-theme-bg-tertiary rounded-lg border border-theme-border overflow-hidden">
                            ...
                            <button
                                onClick={() => setActiveSubTab('multe')}
                                className={`px-4 py-2 text-sm font-medium transition-colors ${activeSubTab === 'multe' ? 'bg-dr7-gold text-black' : 'text-theme-text-muted hover:text-theme-text-primary'}`}
                            >
                                Multe
                            </button>
                            <button
                    ...
                    {multaStep === 'upload' && (
                        <div className="bg-theme-bg-secondary rounded-lg border border-theme-border overflow-hidden">
                            ...
                            <div className="p-5 space-y-4">
                                ...
                                {/* Show extracted data even if driver not found */}
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="bg-theme-bg-secondary rounded-lg border border-green-500/30 overflow-hidden">
                                ...
                                <div className="px-4 pb-4">
                                    <div className="text-[10px] text-theme-text-muted uppercase mb-2">Allegati che verranno inviati</div>
                                    <div className="flex flex-wrap gap-2">
                                        ...
                                        {!driverData.license_urls?.length && (
