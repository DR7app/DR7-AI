...
import { adjustWashReport, periodKeyOf, CHIAVE_OVUNQUE } from '../../../utils/reportTotals'
...
import { caricaReportVeicoli, type BookingDetail, type VehicleReport, type VehicleReportData } from '../../../utils/reportVeicoliPeriodo'
...
export default function ReportsTab({ business = 'rental', businessLabel = 'Noleggio Terra' }: { business?: string; businessLabel?: string } = {}) {
  ...
  async function saveInFlotta(vehicleId: string, dati: DatiFlotta) {
    ...
    try {
      ...
      console.error('[ReportsTab] saveInFlotta failed', err)
      alert('Date in flotta non salvate: ' + (err instanceof Error ? err.message : String(err)))
    } finally {
      setSavingInFlotta(null)
    }
  }
  ...
  async function fetchReport() {
    ...
    try {
      ...
      // SUE correzioni, e poi la somma. 25/09/2026: la strada e' in
      // utils/reportVeicoliPeriodo.ts, la usa anche la tab Veicoli.
      if (activeReport === 'vehicles') {
        const { dati, overrides: ov } = await caricaReportVeicoli(customFrom, customTo, business)
        setOverrides(ov)
        setVehicleData(dati)
        return
      }
  ...
  function getCategorySummary(vehicles: VehicleReport[]) {
    ...
    // 21/09/2026: un mezzo che nel periodo non era in flotta non abbassa la media.
    const presenti = vehicles.filter(v => !v.nonInFlotta)
    const avgUtil = presenti.length > 0
      ? presenti.reduce((s, v) => s + v.utilizationRate, 0) / presenti.length
      : 0
