    ...
    async function sendWhatsApp(bookingsToSend: Booking[]) {
        ...
        try {
            const response = await fetch('/.netlify/functions/send-checkin-checkout-whatsapp', {
                ...
                body: JSON.stringify({ bookings: bookingsToSend, type: 'checkout' }),
            })
                ...
                <>
                    ...
                    <div className="block md:hidden space-y-3">
                        {bookings.map((booking) => {
                            ...
                            return (
                                <div key={booking.id} className={`bg-theme-bg-secondary rounded-lg p-4 space-y-3 ${overdue ? 'border border-red-500/40' : ''}`}>
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <span className={`font-bold text-xl ${overdue ? 'text-red-500' : 'text-dr7-gold'}`}>
                                                {returnTime}
                                            </span>
                                            {overdue && <span className="ml-2 text-xs text-red-400 font-medium">IN RITARDO</span>}
                                    ...
                                    <div className="grid grid-cols-2 gap-2 text-sm">
                                        ...
                                        <div>
                                            ...
                                            <span className="text-theme-text-primary ml-1">{booking.dropoff_location || booking.pickup_location || '-'}</span>
                                        </div>
                                        <div>
                            ...
                            <table className="w-full">
                                ...
                                <tbody className="divide-y divide-theme-border">
                                    {bookings.map((booking) => {
                                        ...
                                        return (
                                            <tr key={booking.id} className={`hover:bg-theme-bg-tertiary transition-colors ${overdue ? 'bg-red-900/20' : ''}`}>
                                                ...
                                                <td className="px-4 py-4">
                                                    <span className="text-theme-text-primary text-sm">{booking.dropoff_location || booking.pickup_location || '-'}</span>
                                                </td>
                                                <td className="px-4 py-4">
