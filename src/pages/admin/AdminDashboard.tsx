...
const InventarioMagazzino = lazyWithRetry(() => import('./components/InventarioMagazzino'))
...
export default function AdminDashboard() {
  ...
  const SECTIONS: { name: string; tabs: SubTab[] }[] = [
    ...
    // 2026-08-14: ordine UNICO per tutti i business, come Noleggio Terra:
    // Prenotazioni, Preventivi, Uscite Straordinarie, Calendario, Contratti,
    // Danni & Penali, Multe, Veicoli (catalogo), Magazzino, GPS Flotta, Tour.
    // Cargos resta SOLO su Terra. Movimenti (solo Aria) chiude la lista.
    ...
    // Il Catalogo Prime Wash era gia' implementato ma non raggiungibile dal
    ...
    { name: 'Marketing', tabs: [
      ...
      // 15/09/2026 (direzione): le prevendite stanno accanto al Codice Sconto,
      // dentro Marketing. Sono la stessa leva commerciale: si vende un
      // vantaggio prima del noleggio. Una voce sola: Catalogo, Vendute e Popup
  ...
  const tabLabels: Record<string, string> = {
    ...
    'multe-terra': 'Multe Noleggio Terra',
    'multe-lavaggio': 'Multe Lavaggio e Meccanica',
    'cargos': 'Cargos',
    'trustera': 'DR7 Trust',
    ...
    'carwash-calendar': 'Calendario Lavaggio & Meccanica',
    'carwash-catalog': 'Catalogo Lavaggio & Meccanica',
    'vehicles': 'Veicoli',
    ...
    <div className="min-h-screen flex bg-theme-bg-secondary">
      ...
      <div className="flex-1 flex flex-col min-h-screen w-full max-w-full">
              ...
              {userMenuOpen && (
                <>
                  ...
                  <div className="absolute right-0 top-12 z-50 w-64 max-w-[calc(100vw-1rem)] bg-theme-bg-secondary border border-theme-border rounded-xl shadow-2xl py-2 text-sm overflow-hidden">
                    ...
                    {/* 24/08: da qui si arriva alla creazione di un account.
                        Gli account si creano in Operatori (invito via email con
        ...
        <main className={`flex-1 bg-theme-bg-secondary ${(activeTab === 'calendar' || activeTab === 'carwash-calendar') ? 'p-0' : 'p-3 sm:p-6 lg:p-8'}`}>
          ...
          {activeTab === 'terra-tours' && <NoleggioServiceTab serviceType="car_rental" view="tours" labels={{ title: 'Noleggio Terra', asset: 'Veicolo', assetPlural: 'Veicoli' }} />}
          {activeTab === 'calendar' && (
            <CalendarTab
          ...
          {activeTab === 'multe-terra' && <GestioneMulteTab business="rental" />}
          {activeTab === 'multe-lavaggio' && <GestioneMulteTab business="car_wash" />}
      ...
      {/* Calendario Giornaliero — riagganciato al bottone in alto a destra. */}
      ...
      {/* La Gestione Allarmi e' passata dentro Centralina Pro > Allarmi.
          Qui resta solo il pulsante per attivare l'audio: serve un gesto
          dell'utente e deve restare a portata di click. */}
